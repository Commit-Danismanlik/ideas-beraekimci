...
import { IConfirmPasswordResetDto } from '../models/Auth.model';

export const ResetPassword = () => {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const { confirmPasswordReset, loading, error } = useAuthContext();
  ...
  useEffect(() => {
    // Firebase action handler'dan gelen oobCode'u al
    // Firebase bazen 'oobCode' bazen 'code' olarak gönderebilir
    const code = searchParams.get('oobCode') || searchParams.get('code');
    if (!code) {
      // Eğer oobCode yoksa hata göster
      return;
    }
    setOobCode(code);
  }, [searchParams]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const dto: IConfirmPasswordResetDto = {
      oobCode: oobCode,
      newPassword: formData.password,
    };

    const result = await confirmPasswordReset(dto);
        ...
        <div className="max-w-md w-full relative z-10">
          <div className="glass-strong rounded-3xl shadow-glow-lg p-8 backdrop-blur-xl">
            <div className="text-center mb-8 animate-fade-in-up">
              ...
              <Link
                to="/forgot-password"
                className="inline-block bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 hover:from-indigo-500 hover:via-purple-500 hover:to-pink-500 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300 transform hover:scale-[1.02] shadow-lg hover:shadow-indigo-500/50"
              >
                Yeni Link İste
              </Link>
            </div>
      ...
      <div className="max-w-md w-full relative z-10">
        <div className="glass-strong rounded-3xl shadow-glow-lg p-8 backdrop-blur-xl">
          <div className="text-center mb-8 animate-fade-in-up">
            ...
            <p className="text-indigo-300 text-lg">Yeni şifrenizi girin</p>
          </div>
          ...
          {success ? (
            <div className="mb-4 bg-green-500/20 border border-green-500/30 backdrop-blur-sm text-green-300 px-4 py-3 rounded-xl animate-fade-in-scale">
              <p className="font-semibold mb-2">Şifre başarıyla değiştirildi!</p>
              <p className="text-sm">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  {formData.password && formData.confirmPassword && formData.password !== formData.confirmPassword && (
                    <p className="text-red-400 text-sm">Şifreler eşleşmiyor</p>
                  )}
                </div>
